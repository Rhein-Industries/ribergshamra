...
pub(crate) enum KeyMaterial {
    ...
    PostQuantum {
        ...
        algorithm: ribergshamra_crypto::sign::PqAlgorithm,
...
/// Opaque, cloneable ribergshamra key data.
///
...
impl KeyData {
    ...
    pub fn from_symmetric_bytes(
        ...
        let material = match algorithm {
            ...
            _ => {
                return Err(ribergshamra_core::Error::Key(format!(
    ...
    pub fn from_x25519(
        ...
        let private = private.map(<[u8; 32]>::try_from).transpose().map_err(|_| {
            ribergshamra_core::Error::Key("X25519 private key must be 32 bytes".into())
        })?;
        let public = <[u8; 32]>::try_from(public).map_err(|_| {
            ribergshamra_core::Error::Key("X25519 public key must be 32 bytes".into())
        })?;
    ...
    pub fn export_public(&self) -> Result<Vec<u8>, ribergshamra_core::Error> {
        self.software_key()?
            .ok_or_else(|| {
                ribergshamra_core::Error::Key(format!(
    ...
    pub fn export_private(&self) -> Result<zeroize::Zeroizing<Vec<u8>>, ribergshamra_core::Error> {
        self.software_key()?
            .ok_or_else(|| {
                ribergshamra_core::Error::Key(format!(
                    "{} key has no software representation",
        ...
        let imported = match self.data.material() {
            KeyMaterial::Rsa {
                ...
                let der = key.to_pkcs8_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("RSA PKCS#8 encode: {e}"))
                })?;
            ...
            KeyMaterial::Rsa { public, .. } => {
                let der = public
                    .to_public_key_der()
                    .map_err(|e| ribergshamra_core::Error::Key(format!("RSA SPKI encode: {e}")))?;
            ...
            KeyMaterial::EcP256 {
                ...
                let der = key.to_pkcs8_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("P-256 PKCS#8 encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_pkcs8_der(
            ...
            KeyMaterial::EcP256 { public, .. } => {
                let der = public.to_public_key_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("P-256 SPKI encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_spki_der(
                    riptering::KeyAlgorithm::Ec(riptering::EcCurve::P256),
                    der.as_bytes(),
            ...
            KeyMaterial::EcP384 {
                ...
                let der = key.to_pkcs8_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("P-384 PKCS#8 encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_pkcs8_der(
            ...
            KeyMaterial::EcP384 { public, .. } => {
                let der = public.to_public_key_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("P-384 SPKI encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_spki_der(
                    riptering::KeyAlgorithm::Ec(riptering::EcCurve::P384),
                    der.as_bytes(),
            ...
            KeyMaterial::EcP521 {
                ...
                let secret =
                    p521::SecretKey::from_slice(key.to_bytes().as_slice()).map_err(|e| {
                        ribergshamra_core::Error::Key(format!("P-521 private conversion: {e}"))
                    })?;
                let der = secret.to_pkcs8_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("P-521 PKCS#8 encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_pkcs8_der(
            ...
            KeyMaterial::EcP521 { public, .. } => {
                let public =
                    p521::PublicKey::from_sec1_bytes(public.to_encoded_point(false).as_bytes())
                        .map_err(|e| {
                            ribergshamra_core::Error::Key(format!("P-521 public conversion: {e}"))
                        })?;
                let der = public.to_public_key_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("P-521 SPKI encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_spki_der(
                    riptering::KeyAlgorithm::Ec(riptering::EcCurve::P521),
                    der.as_bytes(),
            ...
            KeyMaterial::Dsa {
                ...
                let der = key.to_pkcs8_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("DSA PKCS#8 encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_pkcs8_der(
            ...
            KeyMaterial::Dsa { public, .. } => {
                let der = public
                    .to_public_key_der()
                    .map_err(|e| ribergshamra_core::Error::Key(format!("DSA SPKI encode: {e}")))?;
                Some(riptering::SoftwareKey::from_spki_der(
                    riptering::KeyAlgorithm::Dsa,
                    der.as_bytes(),
                ))
            ...
            KeyMaterial::Ed25519 {
                ...
                let der = key.to_pkcs8_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("Ed25519 PKCS#8 encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_pkcs8_der(
            ...
            KeyMaterial::Ed25519 { public, .. } => {
                let der = public.to_public_key_der().map_err(|e| {
                    ribergshamra_core::Error::Key(format!("Ed25519 SPKI encode: {e}"))
                })?;
                Some(riptering::SoftwareKey::from_spki_der(
                    riptering::KeyAlgorithm::Ed25519,
                    der.as_bytes(),
...
fn map_riptering_error(error: riptering::Error) -> ribergshamra_core::Error {
    match error {
        ...
        riptering::Error::Io(error) => ribergshamra_core::Error::Io(error),
        #[allow(unreachable_patterns)]
