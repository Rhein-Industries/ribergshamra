...
impl KeyData {
    ...
    pub fn from_pkcs8_der(
        ...
        der: &[u8],
    ) -> Result<Self, ribergshamra_core::Error> {
    ...
    pub fn from_spki_der(
        ...
        let key =
            riptering::SoftwareKey::from_spki_der(algorithm, der).map_err(map_riptering_error)?;
    ...
    pub fn algorithm(&self) -> riptering::KeyAlgorithm {
        match self.0.as_ref() {
            ...
            Material::Dh(_) => riptering::KeyAlgorithm::Dh,
        }
    }
    ...
    pub fn to_key_value_xml(&self, prefix: &str) -> Option<String> {
        match self.0.as_ref() {
            Material::Software {
                ...
                riptering::KeyAlgorithm::Rsa => rsa_key_value(spki.as_deref()?, prefix),
                riptering::KeyAlgorithm::Ec(curve) => {
                    ec_key_value(raw_public.as_deref()?, curve_uri(curve))
                }
                riptering::KeyAlgorithm::X25519 => ec_key_value(
                    raw_public.as_deref()?,
                    "urn:ietf:params:xml:ns:keyprov:curve:x25519",
                ),
                riptering::KeyAlgorithm::Dsa => dsa_key_value(spki.as_deref()?, prefix),
                _ => None,
            },
    ...
    pub fn software_key(&self) -> Result<Option<riptering::SoftwareKey>, ribergshamra_core::Error> {
    ...
    pub fn export_public(&self) -> Result<Vec<u8>, ribergshamra_core::Error> {
        self.software_key()?
            .ok_or_else(|| ribergshamra_core::Error::Key("key has no provider handle".into()))?
            .export_spki_der()
            .map_err(map_riptering_error)
    }

    pub fn export_private(&self) -> Result<Zeroizing<Vec<u8>>, ribergshamra_core::Error> {
        match self.0.as_ref() {
            Material::Software { key, .. } => key.export_private().map_err(map_riptering_error),
            Material::Dh(dh) => dh
                .private
                .as_ref()
                .map(|value| Zeroizing::new(value.to_vec()))
                .ok_or_else(|| {
                    ribergshamra_core::Error::Key("DH key has no private material".into())
                }),
        }
    }
}
...
impl Key {
    ...
    pub fn software_key(
        &self,
    ) -> Result<Option<ribergshamra_crypto::sign::SigningKey>, ribergshamra_core::Error> {
    ...
    pub fn dh_parameters(
        &self,
    ) -> Result<Option<riptering::DhParameters>, ribergshamra_core::Error> {
        Ok(self
            .software_key()?
    ...
    pub fn ec_public_key_bytes(&self) -> Option<Vec<u8>> {
        match self.data.0.as_ref() {
            Material::Software {
                key, raw_public, ..
            } if matches!(key.algorithm(), riptering::KeyAlgorithm::Ec(_)) => raw_public.clone(),
            _ => None,
        }
    }
}
...
fn classify_public(
    ...
    match (algorithm, component) {
        (riptering::KeyAlgorithm::X25519, component) => (None, component),
        (_, Some(spki)) => {
...
fn rsa_key_value(spki_der: &[u8], prefix: &str) -> Option<String> {
    ...
    let root = pname(ribergshamra_core::ns::node::RSA_KEY_VALUE);
    let modulus_tag = pname(ribergshamra_core::ns::node::RSA_MODULUS);
    let exponent_tag = pname(ribergshamra_core::ns::node::RSA_EXPONENT);
...
fn dsa_key_value(spki_der: &[u8], prefix: &str) -> Option<String> {
    ...
    let mut writer = uppsala::XmlWriter::new();
    let root = pname(ribergshamra_core::ns::node::DSA_KEY_VALUE);
    writer.start_element(&root, &[]);
    for (name, value) in [
        (ribergshamra_core::ns::node::DSA_P, p),
        (ribergshamra_core::ns::node::DSA_Q, q),
        (ribergshamra_core::ns::node::DSA_G, g),
        (ribergshamra_core::ns::node::DSA_Y, y),
    ] {
        let tag = pname(name);
...
fn ec_key_value(public: &[u8], curve: &str) -> Option<String> {
    ...
    writer.start_element(
        ribergshamra_core::ns::node::EC_KEY_VALUE,
        &[("xmlns", ribergshamra_core::ns::DSIG11)],
    );
    writer.empty_element(ribergshamra_core::ns::node::NAMED_CURVE, &[("URI", curve)]);
    writer.start_element(ribergshamra_core::ns::node::PUBLIC_KEY, &[]);
    writer.text(&encoded);
    writer.end_element(ribergshamra_core::ns::node::PUBLIC_KEY);
    writer.end_element(ribergshamra_core::ns::node::EC_KEY_VALUE);
    Some(writer.into_string())
}
...
fn dh_key_value(dh: &DhMaterial) -> String {
    ...
    writer.start_element(
        "xenc:DHKeyValue",
        &[("xmlns:xenc", ribergshamra_core::ns::ENC)],
    );
    for (name, value) in [
...
fn curve_uri(curve: riptering::EcCurve) -> &'static str {
    match curve {
        riptering::EcCurve::P256 => "urn:oid:1.2.840.10045.3.1.7",
        riptering::EcCurve::P384 => "urn:oid:1.3.132.0.34",
        riptering::EcCurve::P521 => "urn:oid:1.3.132.0.35",
    }
}

fn algorithm_name(algorithm: riptering::KeyAlgorithm) -> &'static str {
    match algorithm {
...
fn map_riptering_error(error: riptering::Error) -> ribergshamra_core::Error {
    match error {
        riptering::Error::Key(message) => ribergshamra_core::Error::Key(message),
        riptering::Error::Crypto(message) => ribergshamra_core::Error::Crypto(message),
        error @ riptering::Error::UnsupportedAlgorithm { .. } => {
            ribergshamra_core::Error::UnsupportedAlgorithm(error.to_string())
        }
        error => ribergshamra_core::Error::Crypto(error.to_string()),
    }
}
