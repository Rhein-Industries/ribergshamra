...
fn resolve_agreement_method_kek(
    ...
    let shared_secret = match agreement_alg {
        algorithm::ECDH_ES => {
            ...
            ribergshamra_crypto::keyagreement::ecdh(curve, &originator_public_bytes, &private_key)?
        }
        algorithm::DH_ES => {
            ...
            let private_key = required_software_key(recipient_key)?;
            ribergshamra_crypto::keyagreement::dh_compute(&originator_dh.public_key, &private_key)?
        }
        algorithm::X25519 => {
            ...
            let private_key = required_software_key(recipient_key)?;
            ribergshamra_crypto::keyagreement::x25519(&originator_public_bytes, &private_key)?
        }
        _ => {
        ...
        Some(kdm_id) => {
            ...
            match kdf_uri {
                algorithm::CONCAT_KDF => {
                    ...
                    ribergshamra_crypto::kdf::concat_kdf(&shared_secret, kek_len, &params)?
                }
                algorithm::PBKDF2 => {
                    let params =
                        parse_pbkdf2_params(doc, kdm_id, kek_len, ctx.max_pbkdf2_iterations)?;
                    ribergshamra_crypto::kdf::pbkdf2_derive(&shared_secret, &params)?
                }
                algorithm::HKDF => {
                    let params = parse_hkdf_params(doc, kdm_id, kek_len)?;
                    ribergshamra_crypto::kdf::hkdf_derive(&shared_secret, kek_len, &params)?
                }
                _ => {
                    return Err(Error::UnsupportedAlgorithm(format!(
...
fn resolve_recipient_key<'a>(
    ...
    doc: &Document<'_>,
    agreement_id: NodeId,
) -> Result<&'a ribergshamra_keys::key::Key, Error> {
...
fn required_software_key(
    key: &ribergshamra_keys::key::Key,
) -> Result<riptering::SoftwareKey, Error> {
...
pub(crate) fn resolve_derived_key(
    ...
    match kd_alg {
        algorithm::CONCAT_KDF => {
            ...
            ribergshamra_crypto::kdf::concat_kdf(&master_key_bytes, key_len, &params)
        }
        algorithm::PBKDF2 => {
            let params =
                parse_pbkdf2_params(doc, kd_method_id, key_len, ctx.max_pbkdf2_iterations)?;
            ribergshamra_crypto::kdf::pbkdf2_derive(&master_key_bytes, &params)
        }
        _ => Err(Error::UnsupportedAlgorithm(format!(
...
pub(crate) fn parse_concat_kdf_params(
    doc: &Document<'_>,
    kd_method_id: NodeId,
) -> Result<ribergshamra_crypto::kdf::ConcatKdfParams, Error> {
    let mut params = ribergshamra_crypto::kdf::ConcatKdfParams::default();
...
pub(crate) fn parse_pbkdf2_params(
    ...
    max_iterations: u32,
) -> Result<ribergshamra_crypto::kdf::Pbkdf2Params, Error> {
    ...
    Ok(ribergshamra_crypto::kdf::Pbkdf2Params {
        prf_uri,
        salt,
        iteration_count,
        key_length,
...
pub(crate) fn parse_hkdf_params(
    ...
    kd_method_id: NodeId,
    default_key_len: usize,
) -> Result<ribergshamra_crypto::kdf::HkdfParams, Error> {
