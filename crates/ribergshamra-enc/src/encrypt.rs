...
fn resolve_agreement_method_encrypt(
    ...
    let shared_secret = match agreement_alg {
        algorithm::ECDH_ES => {
            ...
            ribergshamra_crypto::keyagreement::ecdh(curve, &recipient_public_bytes, &private_key)?
        }
        algorithm::DH_ES => {
            ...
            let private_key = required_software_key(originator_key)?;
            ribergshamra_crypto::keyagreement::dh_compute(recipient.public_key(), &private_key)?
        }
        _ => {
            return Err(Error::UnsupportedAlgorithm(format!(
