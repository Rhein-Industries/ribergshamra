...
impl EncContext {
    ...
    /// `rsa-1_5`, `rsa-oaep-mgf1p`, or `rsa-oaep`). ribergshamra checks that the
    ...
    ///
    /// `allowed_algorithms` must list the XML `EncryptionMethod` Algorithm URI(s)
    /// this encryptor is allowed to service. ribergshamra checks the template's
...
mod tests {
    ...
    use ribergshamra_core::algorithm;
    use ribergshamra_keys::KeysManager;
