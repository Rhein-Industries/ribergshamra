...
pub fn ecdh(
    curve: riptering::EcCurve,
    peer_public: &[u8],
    private_key: &riptering::SoftwareKey,
) -> Result<Vec<u8>, Error> {
    riptering::keyagreement::agree(curve, peer_public, private_key)
        .map_err(crate::map_riptering_err)
}

/// Compute an X25519 shared secret through the selected provider.
pub fn x25519(peer_public: &[u8], private_key: &riptering::SoftwareKey) -> Result<Vec<u8>, Error> {
    riptering::keyagreement::agree_x25519(peer_public, private_key)
        .map_err(crate::map_riptering_err)
}
...
pub fn dh_compute(
    ...
    riptering::keyagreement::agree_dh(other_public, private_key).map_err(crate::map_riptering_err)
}
...
mod tests {
    ...
    fn x25519_roundtrip() {
        ...
        let alice_key = riptering::SoftwareKey::from_x25519(
        ...
        let bob_key =
            riptering::SoftwareKey::from_x25519(Some(bob_secret.as_bytes()), bob_public.as_bytes())
                .unwrap();
    ...
    fn x25519_invalid_public_key_length() {
        ...
        let key = riptering::SoftwareKey::from_x25519(Some(&secret), public.as_bytes()).unwrap();
    ...
    fn x25519_invalid_private_key_length() {
        ...
        let err = riptering::SoftwareKey::from_x25519(Some(&short_priv), &pub_key).unwrap_err();
        assert!(
