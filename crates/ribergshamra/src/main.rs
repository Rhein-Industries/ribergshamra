...
//! ribergshamra CLI — XML Security operations (sign, verify, encrypt, decrypt).

use clap::{Parser, Subcommand};
use ribergshamra_core::Error;
use ribergshamra_keys::key::{Key, KeyData, KeyUsage};
...
#[command(
    name = "ribergshamra",
    about = "ribergshamra — Pure Rust XML Security (XML-DSig, XML-Enc, C14N)",
    version
)]
struct Cli {
    #[command(subcommand)]
...
fn cmd_verify(
    ...
    let mut ctx = ribergshamra_dsig::DsigContext::new_permissive(mgr);
    for attr in &id_attr {
    ...
    let result = ribergshamra_dsig::verify::verify(&ctx, &xml)?;
    match &result {
        ribergshamra_dsig::verify::VerifyResult::Valid { references, .. } => {
            if verbose {
        ...
        ribergshamra_dsig::verify::VerifyResult::Invalid { reason } => {
...
fn reference_digest_warning_reason(
    references: &[ribergshamra_dsig::verify::VerifiedReference],
) -> Option<&'static str> {
    let verified_count = references
        .iter()
        .filter(|reference| reference.digest_verified)
        .count();
    reference_digest_warning_reason_from_counts(references.len(), verified_count)
}
...
fn cmd_sign(
    ...
    let signed = ribergshamra_dsig::sign::sign(&ctx, &template_xml)?;
...
fn cmd_decrypt(
    ...
    let mut ctx = ribergshamra_enc::EncContext::new(mgr);
    for attr in &id_attr {
        ctx.add_id_attr(attr);
    }
    ...
    let decrypted = ribergshamra_enc::decrypt::decrypt_to_bytes(&ctx, &xml)?;
...
fn cmd_encrypt(
    ...
    let encrypted = ribergshamra_enc::encrypt::encrypt(&ctx, &template_xml, &data)?;
...
fn cmd_info() -> Result<(), Error> {
    println!("ribergshamra — Pure Rust XML Security Library");
    println!();
...
fn build_keys_manager(
    ...
    if let Some(path) = keys_file_path {
        let keys = ribergshamra_keys::keysxml::load_keys_file(&path)?;
        for key in keys {
            mgr.add_key(key);
        }
    }
    ...
    if let Some(path) = key_path {
        let key = ribergshamra_keys::loader::load_key_file_with_password(&path, password)?;
        mgr.add_key(key);
    }
    ...
    for spec in &key_names {
        if let Some((name, file_str)) = spec.split_once(':') {
            ...
            let mut key =
                match ribergshamra_keys::loader::load_key_file_with_password(&path, password) {
                    Ok(key) => key,
    ...
    if let Some(path) = pkcs12_path {
        ...
        let key = ribergshamra_keys::loader::load_pkcs12(&data, password.unwrap_or(""))?;
        mgr.add_key(key);
    }
