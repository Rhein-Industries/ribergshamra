...
use ribergshamra::c14n as ribergshamra_c14n;
use ribergshamra::crypto as ribergshamra_crypto;
use ribergshamra::xml as ribergshamra_xml;
...
fn main() {
    ...
    let xdoc = ribergshamra_xml::XmlDocument::parse(xml.clone()).unwrap();
    ...
    for reference in references {
        ...
        if let Some(id_val) = uri.strip_prefix('#') {
            if let Some(&node_id) = id_map.get(id_val) {
                let ns = ribergshamra_xml::nodeset::NodeSet::tree_without_comments(node_id, &doc);

                let result = ribergshamra_c14n::canonicalize(
                    &xml,
                    ribergshamra_c14n::C14nMode::Inclusive,
                    Some(&ns),
                    &[] as &[String],
                )
                .unwrap();
                ...
                let digest = ribergshamra_crypto::digest::digest(digest_uri, &result).unwrap();
                use base64::Engine;
