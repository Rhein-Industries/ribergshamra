...
//!
//! ribergshamra **always** rejects documents that contain duplicate ID values
...
//!   `digest_verified` to report whether ribergshamra locally computed and
...
//! attachment bytes were not hashed by ribergshamra. When local digest coverage is
...
//!
//! ```rust,ignore
//! use ribergshamra_dsig::VerifyResult;
//!
...
//! ```
//!
//! The CLI exposes this as `ribergshamra verify --strict --trusted-keys-only`.
//!
...
//!   [`KeysManager`](ribergshamra_keys::KeysManager). Without this, an attacker
...
/// Convert a [`riptering::Error`] into a [`ribergshamra_core::Error`].
fn map_riptering_err(e: riptering::Error) -> ribergshamra_core::Error {
    match e {
