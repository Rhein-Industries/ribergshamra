...
pub fn sign_owned(ctx: &DsigContext, mut result_xml: String) -> Result<String, Error> {
    ...
    for ref_idx in 0..ref_count {
        ...
        let computed = if let Some(computed) = fast_digest {
            ...
            let mut data = if uri.is_empty() {
                ...
                ribergshamra_transforms::TransformData::xml_borrowed(&result_xml, Some(ns))
            } else if let Some(fragment) = ribergshamra_xml::xpath::parse_same_document_ref(uri) {
                if fragment == "xpointer(/)" {
                    ribergshamra_transforms::TransformData::xml_borrowed(&result_xml, None)
                } else {
                    let is_xpointer =
                        ribergshamra_xml::xpath::parse_xpointer_id(fragment).is_some();
                    let frag_id =
                        ribergshamra_xml::xpath::parse_xpointer_id(fragment).unwrap_or(fragment);
                    let resolved_id =
                        ribergshamra_xml::xpath::resolve_id(&cur_doc, &cur_id_map, frag_id)?;
    ...
    let signature = if let Some(ref hsm_signer) = ctx.hsm_signer {
        ...
        let signer_uri = ribergshamra_crypto::sign::riptering_algorithm_uri(hsm_signer.algorithm());
        ...
        let pq_context: Option<Vec<u8>> =
            if ribergshamra_crypto::sign::is_pq_algorithm(&sig_method_uri) {
        ...
        let sig_alg =
            ribergshamra_crypto::sign::from_uri_with_context(&sig_method_uri, pq_context)?;
...
fn compute_reference_digest_via_transform_pipeline(
    ...
    let mut data = if uri.is_empty() {
        ...
        if fragment == "xpointer(/)" {
            ribergshamra_transforms::TransformData::xml_borrowed(&current_xml, None)
        } else {
            let is_xpointer = ribergshamra_xml::xpath::parse_xpointer_id(fragment).is_some();
            let frag_id = ribergshamra_xml::xpath::parse_xpointer_id(fragment).unwrap_or(fragment);
            let resolved_id = ribergshamra_xml::xpath::resolve_id(doc, id_map, frag_id)?;
            let ns = if is_xpointer {
...
fn validate_signature_method(uri: &str) -> Result<&str, Error> {
    ribergshamra_crypto::sign::from_uri(uri)?;
    Ok(uri)
}

fn validate_digest_method(uri: &str) -> Result<&str, Error> {
    ribergshamra_crypto::digest::from_uri(uri)?;
    Ok(uri)
}

fn validate_c14n_method(uri: &str) -> Result<&str, Error> {
    if ribergshamra_c14n::C14nMode::from_uri(uri).is_some() {
        Ok(uri)
    } else {
...
fn reference_node_set_for_fast(
    ...
    let Some(fragment) = ribergshamra_xml::xpath::parse_same_document_ref(uri) else {
        return Ok(None);
    };
...
fn encrypt_session_key_in_template(
    xml: &str,
    signing_key: &ribergshamra_keys::Key,
    ...
    for node_id in doc.descendants(doc.root()) {
        ...
        let kw = ribergshamra_crypto::keywrap::from_uri(alg_uri)?;
