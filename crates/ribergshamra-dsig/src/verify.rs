...
use ribergshamra_crypto::digest;
use ribergshamra_xml::nodeset::NodeSet;
use ribergshamra_xml::xpath;
...
impl VerifyResult {
    ...
    /// references that ribergshamra reports but does not hash itself.
    #[must_use]
    ...
    /// but ribergshamra did not hash the external attachment bytes. Such callers
...
fn verify_signature_node(
    ...
    let hmac_output_length_bits: Option<usize> =
        if ribergshamra_crypto::sign::is_hmac_algorithm(sig_method_uri) {
    ...
    let pq_context: Option<Vec<u8>> = if ribergshamra_crypto::sign::is_pq_algorithm(sig_method_uri)
    {
    ...
    let c14n_signed_info = ribergshamra_c14n::canonicalize_doc(
        doc,
        c14n_mode,
        Some(&signed_info_ns),
        &inclusive_prefixes,
    ...
    if let Some(ref hsm_verifier) = ctx.hsm_verifier {
        ...
        let verifier_uri =
            ribergshamra_crypto::sign::riptering_algorithm_uri(hsm_verifier.algorithm());
    ...
    let mut extracted_key: Option<ribergshamra_keys::Key> = None;
    let key = if ctx.trusted_keys_only {
        ...
        let inline_key = if prefer_anchorable_inline_key {
            ribergshamra_keys::keyinfo::extract_x509_key_value(effective_ki, doc)
                .or_else(|| ribergshamra_keys::keyinfo::extract_key_value(effective_ki, doc))
        } else {
            ribergshamra_keys::keyinfo::extract_key_value(effective_ki, doc)
        };
    ...
    if !ctx.insecure {
        ...
        if needs_x509_validation && !key.x509_chain.is_empty() {
            let config = ribergshamra_keys::x509::CertValidationConfig {
            ...
            ribergshamra_keys::x509::validate_cert_chain(leaf_der, &key.x509_chain, &config)?;
            if ctx.debug {
    ...
    let sig_alg = ribergshamra_crypto::sign::from_uri_with_context(sig_method_uri, pq_context)?;
        ...
        try_fast_reference_bytes(doc, id_map, uri, sig_node, transforms_node)?
    {
        ...
        let (mut data, resolved_node, debug_pre_digest_bytes) = match resolved {
            ResolvedUri::Xml {
                ...
                ribergshamra_transforms::TransformData::xml_borrowed(xml_text, node_set),
                resolved_node,
                true,
            ),
            ResolvedUri::Binary {
                ...
                ribergshamra_transforms::TransformData::Binary(bytes),
                None,
                debug_pre_digest_bytes,
            ),
        };
...
impl ribergshamra_c14n::C14nSink for ReferenceDigestSink {
...
fn try_fast_reference_digest(
    ...
    ribergshamra_c14n::canonicalize_doc_to(
        doc,
        c14n_mode,
        node_set.as_ref(),
        &inclusive_prefixes,
...
fn try_fast_reference_c14n_bytes(
    ...
    ribergshamra_c14n::canonicalize_doc(doc, c14n_mode, node_set.as_ref(), &inclusive_prefixes)
        .map(Some)
}
...
pub(crate) fn apply_transform<'a>(
    ...
    use ribergshamra_transforms::pipeline::{C14nTransform, Transform};

    match uri {
        algorithm::ENVELOPED_SIGNATURE => {
            let t = ribergshamra_transforms::enveloped::EnvelopedSignatureTransform::from_node_id(
                sig_node,
            );
            t.execute(data)
        }
        ...
        algorithm::BASE64 => {
            let t = ribergshamra_transforms::base64_transform::Base64DecodeTransform;
            t.execute(data)
        }
...
fn apply_relationship_transform<'a>(
    ...
    let xml_text = match &data {
        ...
        ribergshamra_transforms::TransformData::Binary(bytes) => String::from_utf8(bytes.clone())
...
fn apply_xpath_transform<'a>(
        ...
        || is_enveloped_xpath_here(xpath_expr, xpath_node, outer_doc)
    {
        ...
        use ribergshamra_transforms::pipeline::Transform;
        let t =
            ribergshamra_transforms::enveloped::EnvelopedSignatureTransform::from_node_id(sig_node);
...
fn try_compound_xpath_filter<'a>(
    ...
    data: ribergshamra_transforms::TransformData<'a>,
    _sig_node: NodeId,
) -> Result<Option<ribergshamra_transforms::TransformData<'a>>, Error> {
    use ribergshamra_xml::nodeset::{NodeSet, NodeSetType};
    ...
    let (xml_text, input_ns) = match data {
        ribergshamra_transforms::TransformData::Xml { xml_text, node_set } => (xml_text, node_set),
        ribergshamra_transforms::TransformData::Binary(bytes) => {
    ...
    Ok(Some(ribergshamra_transforms::TransformData::Xml {
        xml_text,
...
fn apply_parsed_xpath_filter<'a>(
    ...
    Ok(ribergshamra_transforms::TransformData::Xml {
        xml_text,
        node_set: Some(result_ns),
    })
}
...
fn evaluate_simple_xpath(
    ...
    if let Some(name_expr) = expr.strip_prefix("//") {
        ...
        return Ok(NodeSet::from_ids(
            nodes,
            ribergshamra_xml::nodeset::NodeSetType::Normal,
        ));
    }
...
fn try_unwrap_encrypted_key(
    ...
    manager: &ribergshamra_keys::KeysManager,
) -> Result<ribergshamra_keys::Key, Error> {
    ...
    Ok(ribergshamra_keys::Key::new(
        ribergshamra_keys::key::KeyData::from_symmetric_bytes(
            riptering::KeyAlgorithm::Hmac,
            &session_key_bytes,
        )?,
        ribergshamra_keys::key::KeyUsage::Any,
    ))
}
...
fn resolve_kek_from_key_info<'a>(
    ...
    manager: &'a ribergshamra_keys::KeysManager,
) -> Result<&'a ribergshamra_keys::Key, Error> {
...
fn try_resolve_retrieval_method(
    ...
    base_dir: Option<&str>,
    url_maps: &[(String, String)],
) -> Option<ribergshamra_keys::Key> {
    use ribergshamra_keys::key::{Key, KeyUsage};
...
) -> Option<ribergshamra_keys::key::KeyData> {
    use der::Encode;
    use ribergshamra_keys::key::KeyData;
...
mod tests {
    ...
    impl TestDir {
        ...
        fn new() -> Self {
            ...
            for attempt in 0..100_u32 {
                let path = std::env::temp_dir().join(format!(
                    "ribergshamra-dsig-reference-dir-{pid}-{base_nonce}-{attempt}",
                    pid = std::process::id()
                ));
    ...
    fn hmac_keys_manager() -> ribergshamra_keys::KeysManager {
        let mut keys = ribergshamra_keys::KeysManager::new();
        keys.add_key(ribergshamra_keys::Key::new(
            ribergshamra_keys::KeyData::from_symmetric_bytes(
                riptering::KeyAlgorithm::Hmac,
  ...
  <ds:Signature>
    ...
    fn test_verify_all_errors_when_no_signature() {
        ...
        let ctx = DsigContext::new(ribergshamra_keys::KeysManager::new());
  ...
  <ds:Signature xmlns:ds="http://www.w3.org/2000/09/xmldsig#">
    ...
    fn reference_digest_policy_reports_no_digests_when_all_references_are_unverified() {
        let ctx = DsigContext::new_permissive(ribergshamra_keys::KeysManager::new());
    ...
    /// attachment or detached-content digests outside ribergshamra.
    #[test]
    ...
    fn default_ctx() -> DsigContext {
        DsigContext::new_permissive(ribergshamra_keys::KeysManager::new())
    }
    ...
    fn test_validate_rootxmlns_with_external_cert() {
        ...
        let mut mgr = ribergshamra_keys::KeysManager::new();
        let key = ribergshamra_keys::loader::load_x509_cert_pem(cert_pem.as_bytes())
